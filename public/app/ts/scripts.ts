var packages      = <any> document.querySelectorAll('.packages-list__package');
var empty         = document.querySelector('.packages-list__empty');
var search        = document.querySelector('.layout-search__input');
var form          = document.querySelector('.layout-search');
var packagesInfos = [];
var lastQuery     = null;
 ...
 * @return {void}
 */
function each(selector: any, fn: Function) {
...
form.addEventListener('submit', function(event) {
	...
	window.location.href = 'package/' + firstPackage.dataset.id;
});
...
handleEvent('click', 'th', function() {
	...
	var packagesArray = _.toArray(packages);

	// Get index, clean sorting classes
	each(this.parentNode.getElementsByTagName('th'), (element, key) => {
		if (element === this) {
			index = key;
		} else {
